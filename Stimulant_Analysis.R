...
data_2019 <- raw_data_2019 %>% 
  select(AGE, RACERETH, SEX, MSA, PAYTYPER,  # Demographic characteristics
         ...
         MDDO,PRIMCARE,PHYSASST,NPNMW,MHP,OTHPROV,
         CAD, # CAD
         DEPRN, # Depression
         HTN, # Hypertension
         OBESITY, # Obesity
...
### Define Multum Drug Category Codes and ICD Codes of Interest ###
...
data_combined <- data_combined %>% 
  ...
  mutate(ANY_CONTRAINDICATED_CONDITION = case_when(CAD==1 | CEBVD==1 | HTN==1 | SUBSTAB==1 | PREGNANT==1 ~1,
                                                    .default = 0))
...
all_age_any_stim <- subset(weighting_design_namcs,ANY_STIM==1)
all_age_new_stim <- subset(weighting_design_namcs,NEW_STIM==1)
...
table1 <- rbind(#total
                ...
                #HCP characteristics
                svytable(~PRIMCARE+ANY_STIM,  design=all_age_weighted),
                svytable(~MDDO+ANY_STIM,  design=all_age_weighted),
...
#### GLMs ####
summary(svyglm(ANY_STIM ~YEAR+SEX+RACERETH+relevel(factor(AGE_RECODE_3), ref = "18-30")+relevel(factor(PAYTYPER_RECODE), ref = "Private Insurance")+PREGNANT+CAD+DEPRN+HTN+OBESITY+SUBSTAB+CEBVD, design=all_age_weighted,family=quasibinomial()))
summary(svyglm(NEW_STIM ~YEAR+SEX+RACERETH+relevel(factor(AGE_RECODE_3), ref = "18-30")+relevel(factor(PAYTYPER_RECODE), ref = "Private Insurance")+PREGNANT+CAD+DEPRN+HTN+OBESITY+SUBSTAB+CEBVD, design=all_age_weighted,family=quasibinomial()))

tidy(svyglm(ANY_CONTRAINDICATED_CONDITION ~YEAR+SEX+RACERETH+relevel(factor(AGE_RECODE_3), ref = "18-30")+relevel(factor(PAYTYPER_RECODE), ref = "Private Insurance")+MDDO+PRIMCARE+PHYSASST+NPNMW+MHP+OTHPROV, design=all_age_any_stim,family=quasibinomial()), exponentiate = TRUE)
